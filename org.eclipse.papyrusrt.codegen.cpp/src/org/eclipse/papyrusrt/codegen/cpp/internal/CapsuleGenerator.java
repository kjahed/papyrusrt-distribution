...
public class CapsuleGenerator extends SerializableClassGenerator {
	...
	/** Whether border ports have been found. */
	protected Boolean borderPortsFound;
	...
	/**
	 * @return {@code true} if the {@link #capsule} has border ports.
	 */
	protected boolean hasBorderPorts() {
		if (borderPortsFound != null) {
			return borderPortsFound.booleanValue();
		}

		for (Port port : XTUMLRTExtensions.getAllRTPorts(capsule)) {
			if (XTUMLRTUtil.isBorderPort(port)) {
				borderPortsFound = Boolean.TRUE;
				return borderPortsFound;
			}
		}
		borderPortsFound = Boolean.FALSE;
		return borderPortsFound;
	}

	/**
	 * @return number of the {@link #capsule} border ports.
	 */
	protected int borderPortsCount() {
		int count = 0;
		for (Port port : XTUMLRTExtensions.getAllRTPorts(capsule)) {
			if (XTUMLRTUtil.isBorderPort(port)) {
				count++;
	...
	/**
	 * Generate the "create" function for this capsule.
	 * 
	 * <p>
	 ...
	 */
	protected Function generateCreate(CapsuleInstance instance) {
		...
		Function create = new Function(LinkageSpec.STATIC, PrimitiveType.VOID, "create_" + capsule.getName());
		create.add(slot);
		...
		Variable borderPortsPtr = null;
		if (hasBorderPorts()) {
			int numBorderPorts = borderPortsCount();
			borderPortsPtr = new Variable(
					UMLRTRuntime.UMLRTCommsPort.getType().const_().ptr().ptr(),
					"borderPortsPtr",
					new CastExpr(UMLRTRuntime.UMLRTCommsPort.getType().const_().ptr().ptr(),
							StandardLibrary.malloc(new BinaryOperation(
									StandardLibrary.sizeOf(UMLRTRuntime.UMLRTCommsPort.getType().ptr()),
									Operator.MULTIPLY,
									new IntegralLiteral(numBorderPorts)))));
			create.add(borderPortsPtr);
			...
			for (int i = 0; i < numBorderPorts; ++i) {
				create.add(new BinaryOperation(
						new IndexExpr(new ElementAccess(borderPortsPtr), new IntegralLiteral(i)),
						Operator.ASSIGN,
						new AddressOfExpr(
								new IndexExpr(
										new MemberAccess(new ElementAccess(slot), UMLRTRuntime.UMLRTSlot.ports),
		...
		ctorCall.addArgument(borderPortsPtr == null ? StandardLibrary.NULL() : new ElementAccess(borderPortsPtr));
		ctorCall.addArgument(internalPorts == null ? StandardLibrary.NULL() : new ElementAccess(internalPorts));
		// create produces only static instances
		ctorCall.addArgument(BooleanLiteral.TRUE());
	...
	protected Variable generateUMLRTCapsuleClass(Variable subCapsules, CapsuleInstance instance) {
		...
		BlockInitializer init = new BlockInitializer(
				...
				new ElementAccess(generateCreate(instance)),
